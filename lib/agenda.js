...
var Agenda = module.exports = function(config, cb) {
  ...
  if (config.mongo) {
    ...
    this.database(config, config.db.collection, config.db.options, cb);
  }
};
...
Agenda.prototype.database = function(config, table, options, cb) {
  ...
  self._DB.initPoolConn(config, function( result ){
    if(result){
      self._DB.poolConn(function(clientPG, done){
        self.db_init( config, table, clientPG, done, cb );
      });
    }
  });
 ...
 *  @param collection collection name or undefined for default 'agenda_jobs'
 *  NF 20/04/2015
 */
Agenda.prototype.db_init = function( config, table, clientPG, done, cb ){

    var table = config.db.collection || "agenda_jobs",
        self = this;

    self._table = table;
    self._DB.init(table);

    logger.debug("[Agenda] db_init table name: ", table);

    var  querySQL =  'CREATE TABLE IF NOT EXISTS '+ table +' ('
                  ...
                  +  ' "repeatInterval" text,'
                  +  ' "repeatTimezone" timestamp with time zone,'
                  +  ' CONSTRAINT '+ table +'_pkey PRIMARY KEY (id)'
                  + ' );';

    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_id_idx ON '+table+' USING btree (id);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_name_idx ON '+table+' USING btree (name);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_disabled_idx ON '+table+' USING btree (disabled);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_data_idx ON '+table+' USING btree (data);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_type_idx ON '+table+' USING btree (type);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_priority_idx ON '+table+' USING btree (priority);';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_next_run_at_idx ON '+table+' USING btree ("nextRunAt");';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_last_modified_by_idx ON '+table+' USING btree ("lastModifiedBy");';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_locked_at_idx ON '+table+' USING btree ("lockedAt");';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_last_run_at_idx ON '+table+' USING btree ("lastRunAt");';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_last_finished_at_idx ON '+table+' USING btree ("lastFinishedAt");';
    querySQL += 'CREATE INDEX IF NOT EXISTS '+table+'_repeat_interval_idx ON '+table+' USING btree ("repeatInterval");';

    //Callback after query exec.
    var queryOnEnd = function(err, result){
        done();
        if(err){
          logger.error("Error on create table: " + table, err);
          return;
        }

        logger.info("[Agenda] db_init - Success");
        if (cb){
          cb(err);
        }
        self.emit('ready');
    }
    const query = clientPG.query( { text : querySQL}, queryOnEnd );
...
Agenda.prototype.jobs = function( query, cb ){
  var self = this;
  //TODO-FIX: Implementar melhoria para o uso de pesquisas em jobs
  /*
...
Agenda.prototype.now = function(name, data, cb) {
  ...
  job.schedule(new Date(Date.now().valueOf() + 10000));
  job.save(cb);
  return job;
};
...
Agenda.prototype.cancel = function(query, cb) {
  ...
  //TODO:FIX: Atualmente o parametro query está sendo usado apenas recebendo { name: value }, { id: value } e data.{ card_id: value },
  // porém, tem casos usados pelo Agenda.purge() que pode trazer mais parametros. O purge só pode ser
  // usado direto na aplicação, ou seja, não é utiizado pelo Agenda internamente e o mesmo não é usilizado
  // atualmente no projeto, por esse motivo está sendo despriorizada essa melhoria.

  self._DB.deleteMany( query, function( err, result ){

    var data = {};
    data.n = result.rowCount;

    logger.debug("[ deleteMany ] - Success", data);
    if (cb) {
      cb( err, result && data ? data.n : undefined );
    }
  });
};
...
Agenda.prototype.saveJob = function(job, cb) {
  ...
  if (id) {
    self._DB.findAndModify( {id: id},{}, update, {upsert: true, new: true}, props, function(err, result){
        processDbResult(err, result);
    });
    ...
    // Try an upsert.
    self._DB.findAndModify( {name: props.name, type: 'single'},{}, update, {upsert: true, new: true}, props, function(err, result){
      processDbResult(err, result);
    });
    ...
    if( uniqueOpts && uniqueOpts.insertOnly )
      update = { $setOnInsert: props, "unique" : true };

    // Try an upsert.
    self._DB.findAndModify( query,{}, update, {upsert: true, new: true}, props, function(err, result){
        processDbResult(err, result);
    });

  } else {

    self._DB.insertOne( props, function(err, result){
      processDbResult(err, result);
    });
  ...
  function processDbResult(err, result) {
    ...
    logger.debug("[ processDbResult ][ "+props.name+" ] result : ", result);
    if (err) {
      if (fn) {
        return fn(err);
      } else {
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  var querySQL = 'UPDATE '+ self._table +' AS aj'
                +'  SET "lockedAt" = $1'
                +'    FROM ( SELECT * FROM '+ self._table +' AS ajr WHERE ajr."name" = $2 ORDER BY ajr."priority" DESC ) AS ajr'
                +'    WHERE (aj."name" = $3 AND aj."lockedAt" IS NULL AND aj."nextRunAt" <= $4 AND aj."disabled" IS NOT true)'
                +'    OR (aj."name" = $5 AND aj."lockedAt" <= $6 AND aj."disabled" IS NOT true) RETURNING *;';

  var values = [ JSON.stringify(now), jobName, jobName, JSON.stringify(self._nextScanAt), jobName, JSON.stringify(lockDeadline) ];

  var params = {};
  params.querySQL = querySQL;
  params.values = values;

  self._DB.findAndLockNextJob( params, function( err, result) {
    ...
    if (!err && result.value) {
      ...
      params.querySQL = 'SELECT * FROM '+ self._table +' AS aj WHERE aj."id" = $1';
      params.values = [ jobId ];

      self._DB.find( params, function( err, result) {
...
Agenda.prototype._unlockJobs = function(done) {
  ...
  logger.info("Agenda._unlock","Método não implementado por ser chamada apenas manualmente via Agenda.");
  //TODO:FIX: Implementar melhoria para permitir executar a funcionalidade Agenda.stop()
  //VERSAO ORIGINAL AGENDA
...
function processJobs(extraJob) {
  ...
  function lockOnTheFly() {
    ...
    var querySQL = 'UPDATE '+ self._table +' AS aj'
                  +'  SET "lockedAt" = $1'
                  +'    FROM ( SELECT * FROM '+ self._table +' AS ajr WHERE ajr."id" = $2 ) AS ajr'
                  +'    WHERE (aj."id" = $3 AND aj."lockedAt" IS NULL AND aj."nextRunAt" = $4 AND aj."disabled" IS NOT true ) RETURNING *;';

    var values = [ JSON.stringify(now), job.attrs.id, job.attrs.id, JSON.stringify(job.attrs.nextRunAt)];

    logger.debug("[ lockOnTheFly ] Query SQL: ", querySQL.toString());
    logger.debug("[ lockOnTheFly ] Query values: ", values);

    var params = {};
    params.querySQL = querySQL;
    params.values = values;

    self._DB.lockOnTheFly( params, function( err, result) {

      if (result.value) {
        var job = createJob(self, result.value);
