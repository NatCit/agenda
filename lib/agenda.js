...
Agenda.prototype.now = function(name, data, cb) {
  ...
  job.schedule(new Date(Date.now().valueOf() + 1000));
  job.save(cb);
  return job;
};
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  logger.debug("[ findAndLockNextJob ] Query SQL: ", querySQL.toString());
  logger.debug("[ findAndLockNextJob ] Query values: ", values);
